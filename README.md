...
### アジェンダ表示機能
会議のアジェンダ（議題）を画面上に表示できます。ポップアップの「アジェンダ表示」トグルでON/OFFを切り替えられます。
...
1. Google Meetのチャットパネルを開く
2. ポップアップの「📤 全員に共有（チャットに送信）」ボタンをクリック
3. アジェンダデータがチャットに自動送信される（チャットパネルが閉じている場合はクリップボードにコピー）
4. 拡張機能をインストールしている他の参加者は自動でアジェンダを受信し、画面中央に通知が表示される
